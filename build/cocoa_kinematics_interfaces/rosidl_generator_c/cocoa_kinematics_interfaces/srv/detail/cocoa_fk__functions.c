...
cocoa_kinematics_interfaces__srv__CocoaFK_Request__init(cocoa_kinematics_interfaces__srv__CocoaFK_Request * msg)
{
  ...
  if (!sensor_msgs__msg__JointState__init(&msg->jointstate)) {
    cocoa_kinematics_interfaces__srv__CocoaFK_Request__fini(msg);
    return false;
  }
  return true;
}
...
cocoa_kinematics_interfaces__srv__CocoaFK_Request__create()
{
  ...
  cocoa_kinematics_interfaces__srv__CocoaFK_Request * msg = (cocoa_kinematics_interfaces__srv__CocoaFK_Request *)allocator.allocate(sizeof(cocoa_kinematics_interfaces__srv__CocoaFK_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(cocoa_kinematics_interfaces__srv__CocoaFK_Request));
  bool success = cocoa_kinematics_interfaces__srv__CocoaFK_Request__init(msg);
  if (!success) {
...
cocoa_kinematics_interfaces__srv__CocoaFK_Request__Sequence__init(cocoa_kinematics_interfaces__srv__CocoaFK_Request__Sequence * array, size_t size)
{
  ...
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cocoa_kinematics_interfaces__srv__CocoaFK_Request * data = NULL;

  if (size) {
    data = (cocoa_kinematics_interfaces__srv__CocoaFK_Request *)allocator.zero_allocate(size, sizeof(cocoa_kinematics_interfaces__srv__CocoaFK_Request), allocator.state);
    if (!data) {
      return false;
    }
    ...
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = cocoa_kinematics_interfaces__srv__CocoaFK_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      ...
      for (; i > 0; --i) {
        cocoa_kinematics_interfaces__srv__CocoaFK_Request__fini(&data[i - 1]);
      }
...
cocoa_kinematics_interfaces__srv__CocoaFK_Request__Sequence__fini(cocoa_kinematics_interfaces__srv__CocoaFK_Request__Sequence * array)
{
  ...
  if (array->data) {
    ...
    for (size_t i = 0; i < array->capacity; ++i) {
      cocoa_kinematics_interfaces__srv__CocoaFK_Request__fini(&array->data[i]);
    }
...
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cocoa_kinematics_interfaces__srv__CocoaFK_Request__Sequence * array = (cocoa_kinematics_interfaces__srv__CocoaFK_Request__Sequence *)allocator.allocate(sizeof(cocoa_kinematics_interfaces__srv__CocoaFK_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = cocoa_kinematics_interfaces__srv__CocoaFK_Request__Sequence__init(array, size);
  if (!success) {
...
cocoa_kinematics_interfaces__srv__CocoaFK_Request__Sequence__destroy(cocoa_kinematics_interfaces__srv__CocoaFK_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    cocoa_kinematics_interfaces__srv__CocoaFK_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}
...
cocoa_kinematics_interfaces__srv__CocoaFK_Request__Sequence__are_equal(const cocoa_kinematics_interfaces__srv__CocoaFK_Request__Sequence * lhs, const cocoa_kinematics_interfaces__srv__CocoaFK_Request__Sequence * rhs)
{
  ...
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!cocoa_kinematics_interfaces__srv__CocoaFK_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
  ...
  cocoa_kinematics_interfaces__srv__CocoaFK_Request__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(cocoa_kinematics_interfaces__srv__CocoaFK_Request);
    cocoa_kinematics_interfaces__srv__CocoaFK_Request * data =
      (cocoa_kinematics_interfaces__srv__CocoaFK_Request *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!cocoa_kinematics_interfaces__srv__CocoaFK_Request__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          cocoa_kinematics_interfaces__srv__CocoaFK_Request__fini(&data[i]);
        }
        free(data);
        return false;
      }
  ...
  for (size_t i = 0; i < input->size; ++i) {
    if (!cocoa_kinematics_interfaces__srv__CocoaFK_Request__copy(
...
cocoa_kinematics_interfaces__srv__CocoaFK_Response__init(cocoa_kinematics_interfaces__srv__CocoaFK_Response * msg)
{
  ...
  if (!geometry_msgs__msg__Point__init(&msg->positionendeffector)) {
    cocoa_kinematics_interfaces__srv__CocoaFK_Response__fini(msg);
    return false;
  }
  return true;
}
...
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cocoa_kinematics_interfaces__srv__CocoaFK_Response * msg = (cocoa_kinematics_interfaces__srv__CocoaFK_Response *)allocator.allocate(sizeof(cocoa_kinematics_interfaces__srv__CocoaFK_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(cocoa_kinematics_interfaces__srv__CocoaFK_Response));
  bool success = cocoa_kinematics_interfaces__srv__CocoaFK_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
...
cocoa_kinematics_interfaces__srv__CocoaFK_Response__Sequence__init(cocoa_kinematics_interfaces__srv__CocoaFK_Response__Sequence * array, size_t size)
{
  ...
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cocoa_kinematics_interfaces__srv__CocoaFK_Response * data = NULL;

  if (size) {
    data = (cocoa_kinematics_interfaces__srv__CocoaFK_Response *)allocator.zero_allocate(size, sizeof(cocoa_kinematics_interfaces__srv__CocoaFK_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = cocoa_kinematics_interfaces__srv__CocoaFK_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        cocoa_kinematics_interfaces__srv__CocoaFK_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
...
cocoa_kinematics_interfaces__srv__CocoaFK_Response__Sequence__fini(cocoa_kinematics_interfaces__srv__CocoaFK_Response__Sequence * array)
{
  ...
  if (array->data) {
    ...
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      cocoa_kinematics_interfaces__srv__CocoaFK_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
...
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cocoa_kinematics_interfaces__srv__CocoaFK_Response__Sequence * array = (cocoa_kinematics_interfaces__srv__CocoaFK_Response__Sequence *)allocator.allocate(sizeof(cocoa_kinematics_interfaces__srv__CocoaFK_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = cocoa_kinematics_interfaces__srv__CocoaFK_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
...
cocoa_kinematics_interfaces__srv__CocoaFK_Response__Sequence__destroy(cocoa_kinematics_interfaces__srv__CocoaFK_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    cocoa_kinematics_interfaces__srv__CocoaFK_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}
...
cocoa_kinematics_interfaces__srv__CocoaFK_Response__Sequence__are_equal(const cocoa_kinematics_interfaces__srv__CocoaFK_Response__Sequence * lhs, const cocoa_kinematics_interfaces__srv__CocoaFK_Response__Sequence * rhs)
{
  ...
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!cocoa_kinematics_interfaces__srv__CocoaFK_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
  ...
  cocoa_kinematics_interfaces__srv__CocoaFK_Response__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(cocoa_kinematics_interfaces__srv__CocoaFK_Response);
    cocoa_kinematics_interfaces__srv__CocoaFK_Response * data =
      (cocoa_kinematics_interfaces__srv__CocoaFK_Response *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!cocoa_kinematics_interfaces__srv__CocoaFK_Response__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          cocoa_kinematics_interfaces__srv__CocoaFK_Response__fini(&data[i]);
        }
        free(data);
        return false;
      }
  ...
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!cocoa_kinematics_interfaces__srv__CocoaFK_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
